...
|---|---|---|---|
| Key-value | Partition key only | O(1) lookup, one item per key | Sessions, feature flags |
| Wide-column | Partition key + sort key | Range queries within a partition, sorted rows | Feeds, order history, timelines |
| Document | Any key + nested JSON item | Flexible per-item attributes | Product catalog, user profiles |

The "mode" is a data modeling decision, not a configuration switch. Choosing PK + SK with a timestamp clustering key gives you wide-column behavior. Storing a deeply nested JSON item gives you document behavior.

### Secondary Access Patterns

Every NoSQL store is optimized for one primary access pattern. When you need a second, you have three options — in order of preference:

**1. Sort key prefix encoding (cheapest)**
Encode the item type into the sort key. All items for a user stay co-located; a prefix filter separates them.

```
PK: user_123  |  SK: post#2026-05-24T10:00   → post data
PK: user_123  |  SK: like#2026-05-24T09:00   → like data
PK: user_123  |  SK: follow#2026-05-24T08:00 → follow data

Query: WHERE PK = 'user_123' AND SK begins_with 'post#'
```

**2. Global Secondary Index (GSI)**
DynamoDB maintains a separate index with a different partition key. Allows an entirely different access dimension. Costs extra read/write throughput.

```
Main table:   PK = user_id, SK = timestamp  → "give me user's events"
GSI:          PK = event_type, SK = timestamp → "give me all posts globally"
```

**3. Filter expression (avoid on hot paths)**
DynamoDB fetches the entire partition first, then discards non-matching rows in memory. A filter expression reduces response size, not read cost. If a user has 10,000 events and only 50 are posts, you pay for 10,000 reads.

```
# This reads everything for user_123 — then filters. Expensive.
WHERE PK = 'user_123' AND event_type = 'post'  ← filter expression, not a key condition
```

### Single-Table Design (DynamoDB) vs One-Table-Per-Query (Cassandra)

Both DynamoDB and Cassandra are wide-column stores with the same core constraint: no ad-hoc joins, model around the query. But they have opposite conventional patterns for handling multiple access patterns.

| | DynamoDB | Cassandra |
|---|---|---|
| Convention | Single-table design — co-locate all entity types in one table | One table per query — separate physical table per access pattern |
| Multiple access patterns | Sort key prefixes + GSIs | Multiple tables; materialized views |
| Secondary indexes | GSI — maintained by DynamoDB, queries a separate index | Secondary indexes query **every node** — considered dangerous at scale |
| No joins | Multiple entity types in one partition, one round trip | Separate tables mean separate reads |
| Best for | Managed, serverless, variable traffic | On-prem, fine-grained tuning, very high sustained write throughput |

Cassandra secondary indexes are widely avoided by practitioners — they fan out to every node in the cluster. The Cassandra answer to secondary access is to write to multiple tables on insert, each modeled for a different query.

### Concurrency Control Without Transactions

NoSQL still faces the **lost update** problem. A counter is at 10; two clients read 10, both compute 11, both write 11. The result should be 12, but one update vanished. This can happen in any database, SQL or NoSQL.

NoSQL handles this with lighter, more targeted tools instead of wrapping everything in a transaction.

| Tool | How it works | Use when |
|---|---|---|
| Atomic single-item op | "Increment by 1" as one indivisible step, with no read-modify-write gap | Counters, flags |
| Conditional write (compare-and-set) | "Write 11 only if the value is still 10," else fail and retry | Single-item update that depends on the current value |
| Version number | Bump a version on each write; "write only if version == 5" | Optimistic concurrency on a record |
...
A transaction is one all-or-nothing **unit** of work. "Atomic" is only the **A** — a single transaction bundles all four guarantees together, not just atomicity.

Transactions protect **reads**, not only writes:

| Operation | Guarantee it leans on | Risk without it |
|---|---|---|
| Writes | Atomicity | A half-finished change is left behind. |
| Reads | Isolation | You see someone else's half-finished change. |

This is why a read-only report summing many accounts can be wrong if it is not in a transaction: a transfer in flight could be counted on one side but not the other.
...
Isolation is not on or off. It comes in levels, each preventing more of the problems above at the cost of concurrency.
...
Serializable behaves as if transactions ran one-at-a-time in a line. It is the strongest and the slowest, so most databases default to Read Committed for throughput and let you raise the level where correctness demands it.

Mental shortcut: **higher isolation means more correctness and less concurrency.**

### How Isolation Is Enforced: Locks and MVCC

Databases enforce isolation with two main mechanisms. The first is **locks**.

| Lock | Who it allows | Effect |
|---|---|---|
| Exclusive (write) | One writer, nobody else | Others wait their turn. |
| Shared (read) | Many readers, no writer | Readers coexist; writes wait. |
...
### Eventual Consistency vs Partition-Induced Staleness

These produce the same symptom (stale read) but have different causes — a common source of confusion.

| | Cause | When it happens | Lag duration |
|---|---|---|---|
| Eventual consistency (normal) | Async replication lag | **Always** — by design, even during healthy operation | Milliseconds |
| Partition-induced staleness | Network failure cuts off a replica | Only during a failure | Unbounded until network heals |

Eventual consistency is not a failure mode. It is the design choice — replication is async, so a read hitting a replica might be slightly behind. This happens every request in a BASE system.

Partition staleness is what that same design choice looks like when something breaks. The replica can't receive new writes, so it falls further and further behind.
...
| "I'll join two tables." | Joins are implementation, not a requirement. If that join runs on every request at scale, it's a signal to reconsider the store — not a reason to choose it. |
...
- Can I name the four isolation levels and which anomaly each one allows?
- Can I explain how locks (shared vs exclusive) and MVCC enforce isolation?
- Can I explain how NoSQL handles concurrent updates without full transactions (atomic ops, conditional writes, version numbers)?
